...
export default class ArticlesService extends ApiService {
    public async createArticle(article : articlesModel, image : File) {
        ...
        try
        {
            ...
            imageURL = await this.instance.post('https://webserver.cesu.local/api/articles/upload', formData, {
                headers: {
    ...
    public modifyArticle(Article : articlesModel): void {
        ...
        axios.put('https://webserver.cesu.local/api/Articles/'+Article._id, JSON.stringify(Article))
